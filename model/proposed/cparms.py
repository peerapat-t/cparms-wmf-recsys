...
class Generator_CPARMS_Disliked:
    """Generate recommendation signals from disliked-item association rules."""
    ...
    def _cluster_presence(
        ...
        presence = (D @ item_cluster_membership).tocsr()
        if presence.nnz:
    ...
    def fit_transform(self, Y) -> sparse.csr_matrix:
        """Generate an unseen-item disliked-association signal from feedback."""


        D = to_D(Y, dtype=COUNT_DTYPE)
        B = to_B(Y, dtype=COUNT_DTYPE)
        _, item_count = D.shape
        observed_user_mask = np.asarray(B.getnnz(axis=1)).reshape(-1) > 0
        negative_user_mask = np.asarray(D.getnnz(axis=1)).reshape(-1) > 0


        transaction_blocks = [D]
        user_membership = None
        item_cluster_presence = None
        ...
        if self.k_user is not None:
            user_labels, user_cluster_count = self._fit_predict_clusters(
                D, negative_user_mask, self.k_user
            )
            user_membership = _one_hot(user_labels, user_cluster_count)
            transaction_blocks.append(user_membership)
        ...
        if self.K_item is not None:
            D_T = D.T.tocsr()
            item_active = np.asarray(D_T.getnnz(axis=1)).reshape(-1) > 0
            item_labels, item_cluster_count = self._fit_predict_clusters(
                D_T, item_active, self.K_item
            )
            item_membership = _one_hot(item_labels, item_cluster_count)
            item_cluster_presence = self._cluster_presence(D, item_membership)
            transaction_blocks.append(item_cluster_presence)
        ...
        token_offset = item_count
        personalized_signal = (
            D.astype(SIGNAL_DTYPE) @ rule_scores[:item_count]
        )
        generic_signal = sparse.csr_matrix(
            D.shape,
            dtype=SIGNAL_DTYPE,
...
def _net_signal(
    S_liked: sparse.spmatrix,
    S_disliked: sparse.spmatrix,
) -> sparse.csr_matrix:
    """Combine liked and disliked evidence into a signed sparse signal."""
    liked = to_Y(S_liked)
    disliked = to_Y(S_disliked)
    if liked.shape != disliked.shape:
        raise ValueError("S_liked and S_disliked must share the same shape.")
    net = (liked - disliked).tocsr().astype(SIGNAL_DTYPE, copy=False)
    net.sum_duplicates()
    net.eliminate_zeros()
    net.sort_indices()
    return net
...
def _als_factor_update(
    L: sparse.csr_matrix,
    liked_signal: sparse.csr_matrix,
    disliked_signal: sparse.csr_matrix,
    ...
    """Solve one ALS update using feedback and signed CPARMS signals."""
    ...
    for row_idx in range(row_count):
        ...
        liked_start = liked_signal.indptr[row_idx]
        liked_stop = liked_signal.indptr[row_idx + 1]
        liked_idx = liked_signal.indices[liked_start:liked_stop]
        liked_values = liked_signal.data[liked_start:liked_stop]

        disliked_start = disliked_signal.indptr[row_idx]
        disliked_stop = disliked_signal.indptr[row_idx + 1]
        disliked_idx = disliked_signal.indices[disliked_start:disliked_stop]
        disliked_values = disliked_signal.data[disliked_start:disliked_stop]
        ...
        if gamma_like > 0.0 and liked_idx.size:

            liked_factors = fixed_factors[liked_idx]
            system_matrix += MODEL_DTYPE(gamma_like) * (
                liked_factors.T @ liked_factors
            )
            right_hand_side += MODEL_DTYPE(gamma_like) * (
                liked_values @ liked_factors
            )

        if gamma_dislike > 0.0 and disliked_idx.size:

            disliked_factors = fixed_factors[disliked_idx]
            system_matrix += MODEL_DTYPE(gamma_dislike) * (
                disliked_factors.T @ disliked_factors
            )
            right_hand_side += MODEL_DTYPE(gamma_dislike) * (
                -disliked_values @ disliked_factors
            )
...
def _signal_loss(
    ...
    """Compute weighted liked and disliked signal reconstruction losses."""
    ...
    for user_idx in range(signal.shape[0]):
        ...
        if item_idx.size == 0:
            continue
        targets = sign * signal.data[start:stop]
        scores = item_factors[item_idx] @ user_factors[user_idx]
...
class CPARMS:
    ...
    def __init__(
        ...
        self.gamma_like = float(gamma_like)
        self.gamma_dislike = float(gamma_dislike)
        ...
        if not np.isfinite(self.gamma_like) or self.gamma_like < 0.0:
            raise ValueError("gamma_like must be finite and >= 0.")
        if not np.isfinite(self.gamma_dislike) or self.gamma_dislike < 0.0:
            raise ValueError("gamma_dislike must be finite and >= 0.")
    ...
    def fit(
        ...
        """Fit joint-signal factors and infer held-out user factors."""
        ...
        liked_signal = to_Y(S_liked)
        if liked_signal.shape != self.shape:
            raise ValueError(f"S_liked must be shape {self.shape}.")
        disliked_signal = to_Y(S_disliked)
        if disliked_signal.shape != self.shape:
            raise ValueError(f"S_disliked must be shape {self.shape}.")
        ...
        fit_liked = liked_signal[fit_user_mask].tocsr()
        fit_disliked = disliked_signal[fit_user_mask].tocsr()
        fit_L_T = fit_L.T.tocsr()
        fit_liked_t = fit_liked.T.tocsr()
        fit_disliked_t = fit_disliked.T.tocsr()
        ...
        for sweep_idx in range(n_sweeps):
            ...
            if _should_report_sweep(sweep_idx, verbose_every):
                ...
                total_loss = wmf_loss + like_loss + dislike_loss + regularization
                print(
                    f"[Sweep {sweep_idx + 1}/{n_sweeps}] "
                    f"WMF_LOSS: {wmf_loss:.6f} "
                    f"LIKE_LOSS: {like_loss:.6f} "
                    f"DISLIKE_LOSS: {dislike_loss:.6f} "
                    f"L2_SUM: {l2_sum:.6f} "
        ...
        if np.any(cold_user_mask):
            self.P[cold_user_mask] = _als_factor_update(
                ...
                liked_signal[cold_user_mask].tocsr(),
                disliked_signal[cold_user_mask].tocsr(),
                self.Q,
                self.lambda_rate,
                self.alpha,
                self.gamma_like,
