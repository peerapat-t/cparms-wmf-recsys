"""Generate disliked CPARMS signals and fit their weighted MF model."""
...
def _als_factor_update(
    ...
    """Solve one ALS update using feedback and disliked-signal confidence."""
...
def _signal_loss(
    ...
    """Compute the weighted disliked-signal reconstruction loss."""
    ...
    for user_idx in range(signal.shape[0]):
        ...
        if item_idx.size == 0:
            continue
        targets = -signal.data[start:stop]
        scores = item_factors[item_idx] @ user_factors[user_idx]
...
class CPARMS_D:
    """Factorize positive feedback jointly with a disliked CPARMS signal."""
    ...
    def fit(
        ...
        """Fit disliked-signal factors and infer held-out user factors."""
