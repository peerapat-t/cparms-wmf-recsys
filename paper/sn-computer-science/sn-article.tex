...
\abstract{
Weighted matrix factorization (WMF) struggles when interactions are sparse and side information is not always available. We investigate whether patterns derived only from the interaction matrix can improve personalized Top-$K$ recommendation for users with short histories.

We propose CPARMS-WMF, a feature-free framework that clusters users and items, mines single-antecedent rules separately from liked and disliked feedback, and adds the resulting user--item scores to WMF as positive and negative targets. We evaluate it on five Amazon datasets using a global temporal split and five random seeds against ItemPop, Standard-WMF, CoFactor, RME, NeuMF, and LightGCN.

% TODO: results paragraph and conclusion sentence, to be written after the
% re-run without users who have no fit-window interactions.
}
...
The interaction matrix contains patterns beyond the history of any single user or item. Clustering can reveal group-level preferences \cite{mirbakhsh2016leveraging}. Association rule mining can reveal directional relationships, and previous work has combined item clustering with association rule mining to reduce sparsity \cite{najafabadi2017clusteringARM}. Co-occurrence is another source of information. CoFactor adds an item--item co-occurrence term to WMF \cite{liang2016cofactor}. RME extends this idea with three co-occurrence terms: items that users often like together, items that users often dislike together, and users who like the same items \cite{tran2018rme}.

These studies use clustering, association rules, or co-occurrence, but none of them combines clustering, association rule mining, and weighted matrix factorization in a single model. Clustering captures shared preference structure, association rules identify directional relationships, and WMF uses this information for personalized ranking. CPARMS-WMF combines these signals within a single objective and, like RME, uses both liked and disliked feedback.

We propose \textbf{CPARMS-WMF}, a feature-free framework for personalized recommendation under sparse implicit feedback. Its \textbf{Clustering and Pairwise Association Rule Mining Signal (CPARMS)} is derived only from the user--item interaction matrix. CPARMS builds two signals, one from liked feedback and one from disliked feedback. For each signal, it clusters users and items, mines single-antecedent rules from user-cluster, item-cluster, and item-level tokens, and converts those rules into a user--item signal. Two CPARMS terms add these signals to the ALS objective: the liked signal raises predicted scores, and the disliked signal lowers them, each with its own weight.

We evaluate users with at least one interaction in the fit window and at least one positive target in the evaluation window. Users and items that appear only in the evaluation window are excluded from training and the candidate set, and their evaluation events are not scored. Results are reported overall and separately for users with one, two, and three or more interactions in the fit window.

CPARMS-WMF is compared with six feature-free baselines in three groups: a non-personalized popularity baseline (ItemPop); three models that share the WMF backbone, namely Standard-WMF \cite{hu2008implicit}, CoFactor \cite{liang2016cofactor}, and RME \cite{tran2018rme}; and two models with different backbones, namely NeuMF \cite{he2017ncf} and LightGCN \cite{he2020lightgcn}.
...
\item CPARMS combines user and item clustering with pairwise association rule mining. It builds separate signals from liked and disliked feedback, and each signal uses user-cluster, item-cluster, and item-level rules from the transactions.

\item CPARMS is integrated into WMF through two additional terms optimized by alternating least squares.

\item CPARMS-WMF is evaluated on five Amazon domains using five random seeds against six baselines. NDCG@10 is the primary metric, with NDCG@20, NDCG@50, NDCG@100, and NDCG@200 reported as secondary measures. The analysis covers mean performance and per-user paired t-tests, both overall and for users with one, two, and three or more interactions.
\end{enumerate}
...
We first review the popularity baseline and matrix factorization for implicit feedback. We then review methods that add side information, methods that extract additional signals from the interaction matrix, and neural and graph-based recommenders.

\subsection{Non-personalized Recommendation}

Ranking candidate items by global popularity, identically for every user, is the standard non-personalized reference point in recommender evaluation \cite{aggarwal2016recommender,zangerle2022fevr}. We call this baseline ItemPop. Because it does not use individual user histories, a personalized model that cannot outperform it has not demonstrated an accuracy benefit from personalization under this evaluation protocol. Popularity remains a strong Top-$K$ baseline: prior work shows that a naive TopPop method can rival sophisticated personalized algorithms and that highly popular items can skew Top-$K$ accuracy \cite{cremonesi2010performance}.
...
Equation~\eqref{eq:cofactor_objective} keeps the co-occurrence term weighted and the WMF term at unit scale. The original formulation instead applies a relative scale $\ell$ to the WMF term and its user and item regularizers. Dividing the objective by $\ell$ gives Eq.~\eqref{eq:cofactor_objective} with $\gamma_{\mathrm{co}}=1/\ell$. Our hyperparameter search keeps this relationship: we sample $\ell$ and set $\gamma_{\mathrm{co}}=1/\ell$, and we sample $\lambda_{\mathrm{ctx}}$ separately (Section~\ref{sec4_hyperparameter}).

RME follows the same idea but uses three SPPMI terms instead of one \cite{tran2018rme}. The first is the CoFactor term over items that users like together. The second is a term over items that users dislike together, computed in the same way from disliked feedback. The third is a user--user term over users who like the same items. Using the same notation, the RME objective is
\begin{equation}
\begin{aligned}
\mathcal{L}_{\mathrm{RME}}
=&
\sum_{u\in\mathcal{U}}\sum_{i\in\mathcal{I}}
c_{ui}
\left(
...
&+
\gamma^{U}
\sum_{(u,v)\in\mathcal{N}}
\left(
n_{uv}
-\mathbf{p}_u^{\top}\mathbf{w}_v
-e_u-h_v-g^{U}
\right)^2 \\
&+
\lambda
...
\lambda_{\mathrm{ctx}}
\left(
\lVert\mathbf{Z}^{+}\rVert_F^2+
\lVert\mathbf{Z}^{-}\rVert_F^2+
\lVert\mathbf{W}\rVert_F^2
\right).
\end{aligned}
\label{eq:rme_objective}
\end{equation}
Here $m^{+}_{ij}$ and $m^{-}_{ij}$ are the SPPMI values of Eq.~\eqref{eq:sppmi} computed from liked and disliked feedback, $n_{uv}$ is the SPPMI value for users $u$ and $v$ computed from liked feedback, $\mathbf{z}^{+}_j$, $\mathbf{z}^{-}_j$, and $\mathbf{w}_v$ are context factors, and $b$, $d$, $e$, $h$, and $g$ are the biases and global intercepts of each term. The item factor $\mathbf{q}_i$ is shared by the WMF term and both item terms, and the user factor $\mathbf{p}_u$ is shared by the WMF term and the user term. RME therefore uses disliked feedback, but, like CoFactor, it works with co-occurrence between single items or single users rather than group-level patterns.

\subsection{Neural and Graph-Based Recommendation}

Other feature-free models replace the WMF backbone. NeuMF combines a generalized matrix factorization branch and a multilayer perceptron branch, so it can learn non-linear user--item interactions, and it is trained with sampled negative items \cite{he2017ncf}. LightGCN learns user and item embeddings by propagating them over the user--item interaction graph with simple weighted sums, and it is trained with a pairwise ranking loss \cite{he2020lightgcn}. Both models learn only from the interaction matrix, but they are trained by gradient descent instead of closed-form ALS updates, and they do not add an explicit signal for users with short histories.

\subsection{How the Proposed Method Differs}

The feature-free methods reviewed above use one or two of clustering, association rule mining, and weighted matrix factorization. CPARMS-WMF combines all three in one framework: clustering provides group-level preference structure shared across the members of each user or item cluster, pairwise association rules provide directional confidence-weighted information, and WMF provides personalized latent-factor ranking. CoFactor and RME are the closest baselines because they also add terms derived from the interaction matrix to the same WMF objective. The main difference is the type of information. CoFactor and RME regularize item and user factors with co-occurrence between single items or single users, while CPARMS builds user--item targets from item--item, user-cluster--item, and item-cluster--item rules. Like RME, CPARMS uses disliked feedback, but it turns this feedback into a separate signal that lowers the scores of candidate items instead of an item co-occurrence term.

CPARMS uses three token families in each signal: user-cluster, item-cluster, and item-level tokens. User-cluster tokens provide a generic signal shared by users in the same cluster, while the other two provide personalized signals based on each user's own history.

This comparison raises two questions. First, can CPARMS-WMF improve personalized ranking under sparse implicit feedback over the WMF-based baselines, the neural and graph baselines, and the popularity baseline? Second, does this improvement remain for users with very short histories?
...
\textbf{CPARMS-WMF} is designed to improve Top-$K$ recommendation under sparse implicit feedback. It derives two CPARMS signals, one from liked feedback and one from disliked feedback, through clustering and pairwise association rule mining on the user--item interaction matrix.

Three main components make up the framework: (i) interaction-based clustering of users and items, followed by pairwise association rule mining, run once on the liked view and once on the disliked view; (ii) construction of a sparse user--item signal from the rules of each view; and (iii) integration of both signals into a weighted matrix factorization objective optimized by alternating least squares.

\subsection{Notation and Feedback Views}

Let $\mathcal{U}=\{1,\ldots,U\}$ and $\mathcal{I}=\{1,\ldots,I\}$ be the user and item index sets, and let $\mathbf{Y}=[y_{ui}] \in \mathbb{R}_{\ge 0}^{U \times I}$ be the rating matrix of the fit window, with $y_{ui}=0$ when user $u$ has not rated item $i$. From $\mathbf{Y}$ the pipeline derives three binary views and keeps them strictly separate:
\begin{equation}
...
\qquad
\underbrace{\delta_{ui} = \mathbb{I}\!\left[\, 0 < y_{ui} < \theta \,\right]}_{\textstyle \mathbf{D}\;(\text{disliked})},
\qquad \theta=4.
\label{eq:binary}
\end{equation}
With integer ratings from $1$ to $5$, a rating of $5$ counts as liked, ratings of $1$ to $3$ count as disliked, and a rating of $4$ counts as neither. We use $\theta=4$ because Amazon ratings are strongly skewed toward high values. With the more common threshold $\theta=3$, the ratio of liked to disliked ratings in our datasets ranges from $9.9{:}1$ to $40.1{:}1$, which leaves too few disliked ratings for rule mining. With $\theta=4$, the ratio ranges from $2.8{:}1$ to $12.3{:}1$. A rating of $4$ still counts as seen in $\mathbf{B}$.

The liked view $\mathbf{L}$ gives the WMF preference target, the relevance labels for evaluation, and the liked CPARMS signal. The disliked view $\mathbf{D}$ gives only the disliked CPARMS signal. The seen view $\mathbf{B}$ is used to mask candidates: an item is treated as seen after any rating and is not recommended again. For user $u$, let $\Omega_u^{+}=\{i \mid \ell_{ui}=1\}$ denote the liked history, $\Omega_u^{-}=\{i \mid \delta_{ui}=1\}$ the disliked history, $\Omega_u=\{i \mid b_{ui}=1\} \supseteq \Omega_u^{+}\cup\Omega_u^{-}$ the seen set, and $n_u=|\Omega_u|$ the number of fit-window interactions.


\subsection{CPARMS: Clustering and Pairwise Association Rule Construction}

Previous studies show that clustering and association rule mining can improve collaborative filtering on sparse data~\cite{mirbakhsh2016leveraging,najafabadi2017clusteringARM}. CPARMS builds on these ideas to construct two additional sparse signals. The same procedure runs twice, once on the liked view and once on the disliked view, with the same hyperparameters. We describe it for a history view $\mathbf{H}\in\{\mathbf{L},\mathbf{D}\}$ and use the superscripts $+$ and $-$ for the liked and disliked results.

We cluster users and items separately with K-means~\cite{macqueen1967kmeans} on $\mathbf{H}$. Each fit uses K-means++ initialization, $10$ restarts, and the experiment seed. K-means is fitted only to active rows, which contain at least one entry in $\mathbf{H}$, and the fitted model then assigns every row---including an all-zero row---to its nearest cluster. Excluding all-zero rows from fitting prevents identical inactive rows from forming an uninformative cluster. Because every row is assigned, every user belongs to one user cluster. User clustering operates on the rows of $\mathbf{H}$ and produces $K_U$ clusters; the one-hot matrix $\mathbf{U} \in \{0,1\}^{U \times K_U}$ records each user's assignment. Item clustering operates on the rows of $\mathbf{H}^{\top}$ and produces the one-hot assignment matrix $\mathbf{G} \in \{0,1\}^{I \times K_I}$. We then construct a binary item-cluster matrix that records whether a user's history in $\mathbf{H}$ contains any item from each cluster:
\begin{equation}
\mathbf{E} = \mathbb{I}\!\left[\, \mathbf{H}\mathbf{G} > 0 \,\right] \in \{0,1\}^{U \times K_I}.
\label{eq:presence}
\end{equation}
Either clustering step can be disabled, which removes the corresponding token family. When $K_U=1$, all users share the same user-cluster token, which acts as a dataset-wide popularity signal.

For each user, we stack three token families (item-level tokens, user-cluster tokens, and item-cluster tokens) into a single transaction matrix:
\begin{equation}
\mathbf{T} = \big[\, \mathbf{H} \;\big|\; \mathbf{U} \;\big|\; \mathbf{E} \,\big]
\in \{0,1\}^{U \times M}, \qquad M = I + K_U + K_I .
\label{eq:tokens}
\end{equation}
Let $\mathcal{A}$ index all $M$ tokens, and let $\mathcal{R}$ be the item-level tokens, which are the only valid consequents. We use user-cluster and item-cluster tokens only as antecedents, so every rule has an individual item as its consequent.
...
\begin{equation}
W_{ab} = F_{ab}\,\mathrm{conf}(a \rightarrow b).
\label{eq:rule_weight}
\end{equation}
Thus, retained rules carry weight in proportion to their directional reliability, while support and lift serve only as noise filters through Eq.~\eqref{eq:mask}. Collecting the retained rules produces $\mathbf{W} \in \mathbb{R}^{M \times I}$, which encodes the filtered relationships from antecedent tokens to individual-item consequents. On the liked view, a rule $a \rightarrow b$ means that users linked to $a$ often like item $b$; on the disliked view, it means that users linked to $a$ often dislike item $b$.
...
\node[transformstage, right=of s1] (s2) {\textbf{2. Feedback views}\\$\mathbf{L}=\mathbb{I}[y_{ui}>4]$ (liked)\\$\mathbf{D}=\mathbb{I}[0<y_{ui}<4]$ (disliked)\\$\mathbf{B}=\mathbb{I}[y_{ui}>0]$ (seen)\\run steps 3--8 for $\mathbf{H}\in\{\mathbf{L},\mathbf{D}\}$};
\node[clusterstage, right=of s2] (s3) {\textbf{3. K-means on $\mathbf{H}$}\\users: rows of $\mathbf{H}\rightarrow\mathbf{U}$\\items: rows of $\mathbf{H}^{\top}\rightarrow\mathbf{G}$\\$\mathbf{E}=\mathbb{I}[\mathbf{H}\mathbf{G}>0]$};
\node[matrixstage, right=of s3] (s4) {\textbf{4. Token matrix}\\$\mathbf{T}=[\mathbf{H}\mid\mathbf{U}\mid\mathbf{E}]$\\$\mathbf{H}$: item-level token\\$\mathbf{U}$: user-cluster token\\$\mathbf{E}$: item-cluster token};
...
\node[rulestage, left=of s5] (s6) {\textbf{6. Rule scores}\\$W_{ab}=F_{ab}\,\mathrm{conf}(a\rightarrow b)$\\confidence of retained rules};
\node[transformstage, left=of s6] (s7) {\textbf{7. Sum confidences, mask}\\$\tilde{\mathbf{S}}=\mathbf{T}\mathbf{W}$\\$\mathbf{H},\mathbf{E}$ personalized; $\mathbf{U}$ generic\\remove all seen items ($\mathbf{B}$)};
\node[outputstage, left=of s7] (s8) {\textbf{8. Normalize}\\row max /\\log-damped row max\\$\mathbf{S}^{+}$ from $\mathbf{L}$, $\mathbf{S}^{-}$ from $\mathbf{D}$};
\draw[arrow] (s1) -- (s2);
\draw[arrow] (s2) -- (s3);
\draw[arrow] (s3) -- (s4);
\draw[arrow] (s4) -- (s5);
...
interaction matrix~$\mathbf{Y}$ to the normalized liked and disliked signal matrices
$\mathbf{S}^{+}$ and $\mathbf{S}^{-}$. Steps 3--8 run once on the liked view~$\mathbf{L}$ and
once on the disliked view~$\mathbf{D}$; the seen view~$\mathbf{B}$ enters only as the candidate
mask in step~7}
...
Figure~\ref{fig:cparms_pipeline} summarizes the implemented pipeline. Each view produces two sources of information. Rules triggered by user-cluster tokens in $\mathbf{U}$ give a generic score shared by members of the same cluster. Rules triggered by item-cluster tokens in $\mathbf{E}$ or item-level tokens in $\mathbf{H}$ give personalized scores because both token types depend on the user's own history in that view.
...
\begin{equation}
\mathbf{T}^{(p)} = \big[\, \mathbf{H} \;\big|\; \mathbf{0}_{U\times K_U} \;\big|\; \mathbf{E} \,\big],
\qquad
...
\label{eq:signal_sum}
\end{equation}
There is no specific blending parameter; the way the two signals combine depends entirely on token activation. Every user activates one user-cluster token, so the generic signal gives a shared cluster-level score vector to each cluster member. In contrast, the number of active personalized antecedents grows with the user's history in the view, so the personalized signal becomes stronger as that history grows. A user with no history in the view activates only the user-cluster token, so Eq.~\eqref{eq:signal_sum} gives this user only the generic signal of that view.

After summing the confidence scores, we set the score of every seen item to zero using $\mathbf{B}$, so the signal applies only to unseen items. Since the seen set includes all liked and disliked items, none of them retains a signal score.
...
\label{eq:normalize}
\end{equation}
Empty rows remain zero, and $0 \le s_{ui} \le 1$ under either mode. Running this procedure on $\mathbf{L}$ gives the liked signal $\mathbf{S}^{+} = [s^{+}_{ui}]$, and running it on $\mathbf{D}$ gives the disliked signal $\mathbf{S}^{-} = [s^{-}_{ui}]$. Both matrices are sparse and non-negative. We write $\Psi_u^{+} = \{\, i \mid s^{+}_{ui} > 0 \,\}$ and $\Psi_u^{-} = \{\, i \mid s^{-}_{ui} > 0 \,\}$ for each user's sets of signal-supported items; by construction, $\Psi_u^{+} \cap \Omega_u = \Psi_u^{-} \cap \Omega_u = \varnothing$. The two sets can overlap, because the same unseen item can be supported by both liked and disliked rules.

\subsection{CPARMS-WMF: Signal-Regularized Weighted Matrix Factorization}

CPARMS-WMF integrates both signals into a single matrix factorization objective. It adds two CPARMS terms over signal-supported items to implicit-feedback WMF~\cite{hu2008implicit}, similar to the additional SPPMI terms in CoFactor~\cite{liang2016cofactor} and RME~\cite{tran2018rme}. We compute interaction scores as the inner product of latent factors:
\begin{equation}
...
\label{eq:pref_conf}
\end{equation}
where $\alpha \ge 0$ sets the extra confidence placed on positive interactions. Disliked and neutral ratings therefore have $\pi_{ui}=0$ and unit confidence in the WMF term, as in Standard-WMF. The same liked view defines positives for WMF training, for the SPPMI term of CoFactor and the liked SPPMI terms of RME, and for evaluation relevance.

The training objective combines a confidence-weighted WMF term over all user--item pairs, a $\gamma^{+}$-weighted liked CPARMS term, a $\gamma^{-}$-weighted disliked CPARMS term, and an $\ell_2$ penalty,
\begin{equation}
\begin{aligned}
\mathcal{L} =\;
& \sum_{u\in\mathcal{U}}\sum_{i\in\mathcal{I}}
...
\big(s^{+}_{ui} - \hat{y}_{ui}\big)^2 \\[2pt]
& +\;
\gamma^{-} \sum_{u\in\mathcal{U}}\sum_{i \in \Psi_u^{-}}
\big(-s^{-}_{ui} - \hat{y}_{ui}\big)^2
\;+\; \lambda\Big( \lVert \mathbf{P} \rVert_F^2 + \lVert \mathbf{Q} \rVert_F^2 \Big).
\end{aligned}
\label{eq:objective}
\end{equation}
The first term is the standard implicit WMF loss. It pulls predictions toward $1$ on liked items with high confidence $1+\alpha$ and toward $0$ on all remaining pairs with unit confidence. The second term pulls predictions on liked-signal items toward $s^{+}_{ui}$, and the third term pulls predictions on disliked-signal items toward $-s^{-}_{ui}$. Every signal-supported item lies outside the seen set, so neither signal competes with an observed rating. For an unseen item supported only by liked rules, the WMF term pulls the prediction toward $0$ while the liked term pulls it up, so the item scores higher than other unobserved items. For an unseen item supported only by disliked rules, the disliked term pulls the prediction below $0$, so the item scores lower than other unobserved items. When both signals support the same item, both terms are active, and for fixed factors the three terms on that entry are minimized at $\hat{y}_{ui}=(\gamma^{+}s^{+}_{ui}-\gamma^{-}s^{-}_{ui})/(1+\gamma^{+}+\gamma^{-})$. The two CPARMS terms therefore provide additional information: each signal-supported entry receives a nonzero target outside the user's observed history, whereas the implicit term alone would treat it only as an unobserved zero. Users with short histories gain extra least-squares constraints wherever the rules support unseen candidates. The hyperparameters $\gamma^{+}$ and $\gamma^{-}$ set the strength of the two signal terms, and setting either weight to $0$ removes that term. The coefficient $\lambda$ is the $\ell_2$ penalty.
...
+ \gamma^{+} \!\sum_{i \in \Psi_u^{+}}\! s^{+}_{ui}\,\mathbf{q}_i
- \gamma^{-} \!\sum_{i \in \Psi_u^{-}}\! s^{-}_{ui}\,\mathbf{q}_i .
\end{aligned}
\label{eq:normal_full}
\end{equation}
Following the standard implicit-feedback decomposition, we expand the confidence-weighted terms around the unit-confidence baseline. As a result, we only need to sum the contributions from liked and signal-supported items for each user:
\begin{equation}
\begin{aligned}
...
\end{equation}
We precompute the shared term $\mathbf{Q}^{\top}\mathbf{Q}$ once per sweep, so each user update costs only the rank-one contributions of that user's liked and signal-supported items. Item-factor updates follow the same procedure with the roles of users and items swapped. We initialize both factor matrices with small-variance random values drawn from a fixed seed, and the algorithm then alternates user and item sweeps for a fixed number of iterations. At inference, we mask all seen items and rank the remaining items for each user by $\hat{y}_{ui} = \mathbf{p}_u^{\top}\mathbf{q}_i$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% REVISION PROGRESS (2026-09-25)
%% DONE (revised for the liked/disliked method and the new baselines):
%%   Abstract, Keywords, Introduction, Related Work,
%%   Proposed Method up to the end of Sec. 3.5 (ALS) -- this point.
%%   Cold-start / new-user claims removed; subsection 3.6 deleted.
%% NOT YET REVISED (everything below this comment is still the old text):
%%   Experimental Setup, Results, tables, Discussion, Limitations,
%%   Conclusion. Old items to replace: theta=3, MostPop, three baselines,
%%   old numbers and old search-space table.
%% OPEN ITEMS:
%%   - Decision: users with no fit-window interactions (interaction_0)
%%     are removed from the experiment; re-run tuning and final runs.
%%   - Abstract results paragraph removed (TODO); rewrite after re-run.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We choose model hyperparameters by randomized search~\cite{bergstra2012random}. For each seed, we sample $30$ configurations per model once and reuse them across all datasets. Every tuned model has a separate reproducible random stream and independently draws all of its hyperparameters, including factorization parameters ($K$, $\lambda$, ALS sweeps, and $\alpha$) and any model-specific parameters. Models use the same candidate values where a hyperparameter has the same meaning, but values are not forced to match within a trial. The experiment seed controls both factor initialization and K-means. For each dataset--seed pair, we keep each model's configuration with the best validation NDCG@10, then retrain the model on the combined training-plus-validation set before testing.
...
User clusters $K_U$       & \{None, 1, 2, 3, 5\}               & CPARMS  & user clusters; None disables clustering \\
Item clusters $K_I$       & \{None, 1, 2, 3, 5\}               & CPARMS  & item clusters; None disables clustering \\
...
\begin{tabular}{lccc}
\toprule
Positive interaction group & vs. MostPop & vs. Standard-WMF & vs. CoFactor \\
\midrule
All users & 25 & 24 & 24 \\
...
Table~\ref{tab:ndcg_group} divides users by the number of positive interactions. For users with one positive interaction, CPARMS-WMF beats Standard-WMF in $21/25$ runs and CoFactor in $19/25$. Its largest mean gains over Standard-WMF occur on Industrial \& Scientific ($+142.6\%$) and Instruments ($+68.2\%$). The gains are not uniform, however. On Luxury Beauty, the mean for users with one positive interaction is lower than both WMF baselines ($9.24$ versus $9.53$ and $9.79$), with only $2/5$ seed wins against each; on Music, it is lower than CoFactor. Results against MostPop are mixed: CPARMS-WMF wins $13/25$ runs for users with one positive interaction and has the best mean for these users only on Pantry. The evidence therefore supports an improvement over the two WMF baselines in most runs for users with one positive interaction, but not on every dataset and not consistently over popularity.

At two interactions, CPARMS-WMF wins $22/25$ against Standard-WMF, $23/25$ against CoFactor, and $18/25$ against MostPop, and has the best mean on three datasets; the two remaining best means go to MostPop on Industrial \& Scientific and CoFactor on Pantry. At three or more interactions, it has the best mean on all five datasets and wins $25/25$ against MostPop, $23/25$ against Standard-WMF, and $24/25$ against CoFactor. This pattern fits the method design: item-level and liked-item-cluster antecedents provide more user-specific information as $n_u$ grows.

\subsection{Computational Cost}

Table~\ref{tab:runtime} reports training time measured on a machine with an AMD Ryzen 5 7535U processor and 32~GB of RAM. CPARMS-WMF costs more to train than either WMF baseline: total time ranges from $0.334$ minutes on Luxury Beauty to $8.818$ minutes on Instruments. This is $3.1$--$14.2\times$ the training time of Standard-WMF and $2.1$--$6.6\times$ that of CoFactor. Most of the extra time comes from denser signal-regularized ALS updates rather than signal construction.
...
Luxury Beauty & MostPop & 0.25 $\pm$ 0.00 & 1.09 $\pm$ 0.00 & 0.00 $\pm$ 0.00 & 0.18 $\pm$ 0.00 \\
              ...
              & CoFactor & \underline{4.33} $\pm$ 0.26 & \textbf{9.79} $\pm$ 0.41 & 5.31 $\pm$ 0.72 & \underline{3.37} $\pm$ 0.25 \\
...
Industrial \& Scientific & MostPop & \underline{2.58} $\pm$ 0.00 & \textbf{6.87} $\pm$ 0.00 & \textbf{4.83} $\pm$ 0.00 & 1.33 $\pm$ 0.00 \\
              ...
              & CoFactor & 1.86 $\pm$ 0.04 & 2.78 $\pm$ 0.22 & 1.98 $\pm$ 0.29 & \underline{1.68} $\pm$ 0.09 \\
...
Pantry        & MostPop & 1.44 $\pm$ 0.00 & \underline{2.11} $\pm$ 0.00 & 1.72 $\pm$ 0.00 & 1.27 $\pm$ 0.00 \\
              ...
              & CoFactor & 1.78 $\pm$ 0.17 & 1.54 $\pm$ 0.10 & \textbf{1.81} $\pm$ 0.33 & 1.82 $\pm$ 0.17 \\
...
Music         & MostPop & 0.44 $\pm$ 0.00 & 0.59 $\pm$ 0.00 & 0.43 $\pm$ 0.00 & 0.41 $\pm$ 0.00 \\
              ...
              & CoFactor & \underline{1.74} $\pm$ 0.05 & \textbf{1.87} $\pm$ 0.21 & \underline{2.73} $\pm$ 0.12 & \underline{1.53} $\pm$ 0.05 \\
...
Instruments   & MostPop & \underline{2.11} $\pm$ 0.00 & \textbf{4.58} $\pm$ 0.00 & \underline{2.93} $\pm$ 0.00 & 1.56 $\pm$ 0.00 \\
              ...
              & CoFactor & 2.03 $\pm$ 0.07 & 2.35 $\pm$ 0.21 & 2.22 $\pm$ 0.09 & \underline{1.94} $\pm$ 0.10 \\
...
Luxury Beauty & MostPop & 0.25 $\pm$ 0.00 & 1.43 $\pm$ 0.00 & 2.94 $\pm$ 0.00 & 3.49 $\pm$ 0.00 & 4.42 $\pm$ 0.00 \\
              ...
              & CoFactor & \underline{4.33} $\pm$ 0.26 & \underline{4.97} $\pm$ 0.26 & \underline{6.09} $\pm$ 0.19 & \underline{6.90} $\pm$ 0.15 & \underline{7.97} $\pm$ 0.15 \\
...
Industrial \& Scientific & MostPop & \underline{2.58} $\pm$ 0.00 & \underline{3.01} $\pm$ 0.00 & \underline{3.81} $\pm$ 0.00 & \underline{4.54} $\pm$ 0.00 & \underline{5.40} $\pm$ 0.00 \\
              ...
              & CoFactor & 1.86 $\pm$ 0.04 & 2.35 $\pm$ 0.06 & 3.23 $\pm$ 0.14 & 4.02 $\pm$ 0.14 & 4.98 $\pm$ 0.19 \\
...
Pantry        & MostPop & 1.44 $\pm$ 0.00 & 2.07 $\pm$ 0.00 & 3.20 $\pm$ 0.00 & 4.47 $\pm$ 0.00 & \underline{5.90} $\pm$ 0.00 \\
              ...
              & CoFactor & 1.78 $\pm$ 0.17 & 2.35 $\pm$ 0.14 & 3.39 $\pm$ 0.11 & 4.44 $\pm$ 0.14 & 5.71 $\pm$ 0.13 \\
...
Music         & MostPop & 0.44 $\pm$ 0.00 & 0.64 $\pm$ 0.00 & 0.98 $\pm$ 0.00 & 1.51 $\pm$ 0.00 & 2.00 $\pm$ 0.00 \\
              ...
              & CoFactor & \underline{1.74} $\pm$ 0.05 & \underline{2.22} $\pm$ 0.09 & \underline{3.06} $\pm$ 0.07 & 3.97 $\pm$ 0.09 & 5.06 $\pm$ 0.10 \\
...
Instruments   & MostPop & \underline{2.11} $\pm$ 0.00 & 2.59 $\pm$ 0.00 & 3.60 $\pm$ 0.00 & 4.46 $\pm$ 0.00 & 5.42 $\pm$ 0.00 \\
              ...
              & CoFactor & 2.03 $\pm$ 0.07 & \underline{2.64} $\pm$ 0.05 & \underline{3.65} $\pm$ 0.08 & 4.60 $\pm$ 0.13 & 5.69 $\pm$ 0.19 \\
...
Luxury Beauty & MostPop & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 \\
              ...
              & CoFactor & 0.000 $\pm$ 0.000 & 0.162 $\pm$ 0.019 & 0.162 $\pm$ 0.019 \\
...
Industrial \& Scientific & MostPop & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 \\
              ...
              & CoFactor & 0.001 $\pm$ 0.000 & 0.533 $\pm$ 0.181 & 0.534 $\pm$ 0.181 \\
...
Pantry        & MostPop & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 \\
              ...
              & CoFactor & 0.002 $\pm$ 0.000 & 0.629 $\pm$ 0.174 & 0.631 $\pm$ 0.174 \\
...
Music         & MostPop & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 \\
              ...
              & CoFactor & 0.003 $\pm$ 0.000 & 1.145 $\pm$ 0.271 & 1.148 $\pm$ 0.271 \\
...
Instruments   & MostPop & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 & 0.000 $\pm$ 0.000 \\
              ...
              & CoFactor & 0.003 $\pm$ 0.000 & 1.341 $\pm$ 0.279 & 1.344 $\pm$ 0.279 \\
...
CoFactor is the closest baseline because both models are feature-free extensions of the same WMF objective. CoFactor slightly exceeds Standard-WMF on four of the five dataset means, whereas CPARMS-WMF exceeds CoFactor on all five means and in $24/25$ runs. Under this evaluation, CPARMS-WMF outperforms CoFactor.
...
The advantage over the WMF baselines grows as the number of positive interactions increases. At two interactions, CPARMS-WMF wins $22/25$ comparisons against Standard-WMF and $23/25$ against CoFactor. At three or more interactions, it has the best mean on all datasets. However, MostPop beats both WMF baselines overall on Industrial \& Scientific and Instruments.
...
The size and source of the gain vary across datasets. Music has the smallest CPARMS-WMF margin over Standard-WMF ($+6.4\%$), while Industrial \& Scientific has the largest ($+58.4\%$). The strongest competitor for users with one positive interaction also varies by domain: CoFactor on Luxury Beauty and Music, while popularity remains difficult to beat for these users on Industrial \& Scientific and Instruments. Pantry is the only dataset where CPARMS-WMF has the best mean for users with one positive interaction.
...
\begin{enumerate}
\item \textbf{Popularity stays competitive at the shortest histories.} CPARMS-WMF improves over Standard-WMF and CoFactor in most comparisons for users with one positive interaction. However, MostPop has the best mean for users with one positive interaction on Industrial \& Scientific and Instruments and the best mean for users with two positive interactions on Industrial \& Scientific. As a result, the method improves on the WMF baselines but does not always beat a non-personalized baseline.
