...
\title[CPARMS-WMF]{
CPARMS-WMF: A Clustering and Pairwise Association Rule Mining Signal for Weighted Matrix Factorization
}
...
\abstract{
Weighted matrix factorization (WMF) struggles when interactions are sparse and side information is unavailable. We investigate whether patterns derived only from the interaction matrix can improve personalized Top-$K$ recommendation for users with short positive histories.
...
CPARMS-WMF improves ranking over both WMF baselines overall and in most comparisons for one-interaction users, but popularity remains competitive for the shortest histories.
}
...
The interaction matrix contains patterns beyond the history of any single user or item. Clustering can reveal group-level preferences \cite{mirbakhsh2016leveraging}; item co-occurrence can strengthen latent-factor learning \cite{liang2016cofactor}; and association rule mining can reveal directional relationships \cite{alsalama2015hybrid}. Previous work has combined item clustering with association rule mining to reduce sparsity \cite{najafabadi2017clusteringARM}, and CFCAI applies cluster-level rule mining to cold-start recommendation \cite{khaledian2025cfcai}.
...
Previous studies show that clustering and association rule mining can improve collaborative filtering on sparse data~\cite{mirbakhsh2016leveraging,alsalama2015hybrid,najafabadi2017clusteringARM}. CPARMS builds on these ideas to construct an additional sparse signal using only positive interactions.
...
\begin{enumerate}
\item \textbf{Popularity stays competitive at the shortest histories.} CPARMS-WMF improves over Standard-WMF and CoFactor-WMF in most comparisons for users with one positive interaction. However, MostPop has the best mean for users with one positive interaction on Industrial \& Scientific and Instruments and the best mean for users with two positive interactions on Industrial \& Scientific. As a result, the method improves on the WMF baselines at short histories but does not always beat a non-personalized baseline.
