...
% See section 11 of the Springer Nature LaTeX template User Manual for version history.
...
\IfFileExists{Orcidlogo.eps}{}{%
  \renewcommand{\orcidlogo}{\textcolor[HTML]{A6CE39}{\textbf{\footnotesize iD}}}%
}
...
\title[CPARMS-WMF under Sparse Implicit Feedback]{
CPARMS-WMF: A Clustering and Pairwise Association Rule Mining Signal for Weighted Matrix Factorization under Sparse Implicit Feedback
}
...
\author[2]{\fnm{Ababa} \sur{Lekakabna}}

\author*[1]{\fnm{Professor Ababa} \sur{Ssd, PhD}}\email{ababa.ssd@example.com}
...
\abstract{
Weighted matrix factorization (WMF) struggles when interaction data are sparse. Side information can help, but user and item attributes are not always available. We examine whether patterns on the interaction matrix alone can help the limited history of an individual user.

We propose CPARMS-WMF, a feature-free framework that combines WMF with a Clustering and Pairwise Association Rule Mining Signal (CPARMS). CPARMS clusters users and items from positive feedback, mines single-antecedent rules over user-cluster, liked-item-cluster, and item-level tokens, and uses the resulting scores as an additional target. We evaluate CPARMS-WMF on five Amazon datasets with five random seeds and compare it with MostPop, Standard-WMF, and CoFactor-WMF.

CPARMS-WMF wins $73/75$ baseline comparisons across dataset--seed runs: $25/25$ against MostPop and $24/25$ against each WMF baseline. On all five datasets, CPARMS-WMF achieves higher mean NDCG@10 than Standard-WMF ($+6.4\%$ to $+58.4\%$), CoFactor-WMF ($+5.3\%$ to $+55.8\%$), and MostPop ($+12.2\%$ to $+1724.4\%$). For users with one positive interaction, it wins $21/25$ against Standard-WMF and $19/25$ against CoFactor-WMF, but only $13/25$ against MostPop.

CPARMS-WMF improves over Standard-WMF and CoFactor-WMF. It also performs better in most comparisons for users with one positive interaction, but the popularity baseline remains competitive for this group.
}
...
The interaction matrix contains patterns beyond the history of any single user or item. Clustering can reveal group-level preferences \cite{mirbakhsh2016leveraging}; item co-occurrence can strengthen latent-factor learning \cite{liang2016cofactor}; and association rule mining can reveal directional relationships through support and confidence \cite{alsalama2015hybrid}. Previous work has combined item clustering with association rule mining to reduce sparsity \cite{najafabadi2017clusteringARM}, and CFCAI applies cluster-level rule mining to cold-start recommendation \cite{khaledian2025cfcai}.

These studies motivate an integrated all clustering, association rule mining, and weighted matrix factorization in a single model. Clustering captures shared preference structure, association rules and find directional rules, and WMF uses this information for personalized ranking. CPARMS-WMF combines these complementary functions within a single objective.

We propose \textbf{CPARMS-WMF}, a feature-free framework for personalized recommendation under sparse implicit feedback. Its \textbf{Clustering and Pairwise Association Rule Mining Signal (CPARMS)} is derived only from the user--item interaction matrix. CPARMS clusters users and items by positive-interaction patterns, mines single-antecedent rules from user-cluster, liked-item-cluster, and item-level tokens, and converts those rules into a sparse user--item signal. An additional reconstruction term incorporates this signal into the ALS objective.
...
\begin{enumerate}
\item We introduce CPARMS-WMF, a feature-free method for recommendation under sparse implicit feedback that not requires any user attributes ir item metadata.
...
\item We integrate the CPARMS signal into WMF through an additional reconstruction term optimized by alternating least squares.

\item We evaluate CPARMS-WMF on five Amazon domains using five random seeds. NDCG@10 is the primary metric, with NDCG@20, NDCG@50, NDCG@100, and NDCG@200 reported as secondary measures. The analysis covers mean performance and pairwise dataset--seed comparisons against all three baselines, both overall and for users with one, two, and three or more positive interactions. It also identifies where CPARMS-WMF improves over the WMF baselines and where popularity remains competitive.
\end{enumerate}
...
Weighted matrix factorization (WMF) extends MF to implicit-feedback data by modeling binary preference alongside confidence weights. WMF learns its latent factors by minimizing a confidence-weighted reconstruction loss, most often with alternating least squares \cite{hu2008implicit}.

Both methods remain sensitive to sparsity: when observed interactions are few, user representations rely on limited information, estimates become unreliable, and Top-$K$ ranking quality drops \cite{roy2022systematic}. This problem motivates additional signals derived from interactions.

\subsection{Non-personalized Recommendation}

Ranking candidate items by global popularity, identically for every user, is the standard non-personalized reference point in offline recommender evaluation \cite{aggarwal2016recommender,zangerle2022fevr}. Because this baseline does not use individual user histories, a personalized model that cannot outperform it has not demonstrated an accuracy benefit from personalization under this evaluation protocol. Under strong sparsity, popularity remains a strong baseline because items that are popular in the fit window are likely to reappear in a future evaluation window \cite{cremonesi2010performance}, and Section~\ref{sec5} shows that it outperforms Standard-WMF on two of our five datasets.
...
Unlike hybrid methods that rely on external attributes, feature-free methods draw additional signals only from the user--item interaction matrix. Clustering-based approaches identify preference structures shared by users or items \cite{mirbakhsh2016leveraging}, while association-rule approaches mine directional item relationships from transaction data \cite{alsalama2015hybrid}. Some studies combine two techniques or connect one of them to matrix factorization. Clustering is used to reduce the dimensionality of the item space, followed by association rule mining \cite{najafabadi2017clusteringARM}. CFCAI mines rules between clusters to support cold-start recommendation \cite{khaledian2025cfcai}. CoFactor instead adds an item--item co-occurrence term to the factorization objective, supplying information for items with few observed interactions \cite{liang2016cofactor}. To our knowledge, none of the studies reviewed above integrates clustering, association rule mining, and weighted matrix factorization within one model.

CoFactor is a relevant matrix-factorization baseline because it links implicit-feedback WMF with an item--item co-occurrence signal derived from the same interaction matrix \cite{liang2016cofactor}. The original model jointly reconstructs the user--item interaction matrix and an item--item shifted positive pointwise mutual information (SPPMI) matrix. Using the notation of this paper, a CoFactor-style objective is
\begin{equation}
\begin{aligned}
...
\label{eq:binary}
\end{equation}
$\mathbf{L}$ is the positive feedback (``liked'') view: at $\theta=3$, integer ratings of four or higher count as positive. $\mathbf{B}$ is the ``seen'' view, recording every observed rating at any level.
...
\end{equation}
Thus, selected rules carry weight in proportion to their directional reliability, while support and lift serve only as noise filters through Eq.~\eqref{eq:mask}. Collecting the selected rules produces $\mathbf{W} \in \mathbb{R}^{M \times I}$, which encodes the filtered relationships from antecedent tokens to individual-item consequents.
...
\node[rulestage, left=of s5] (s6) {\textbf{6. Rule scores}\\$W_{ab}=D_{ab}\,\mathrm{conf}(a\rightarrow b)$\\confidence of selected rules};
...
CPARMS-WMF integrates the signal into a single matrix-factorization objective. It adds a reconstruction term over signal-supported items to implicit-feedback WMF~\cite{hu2008implicit}, similar to the additional reconstruction regularizer in CoFactor~\cite{liang2016cofactor}. We compute interaction scores as the inner product of latent factors:
\begin{equation}
...
\label{eq:pref_conf}
\end{equation}
where $\alpha \ge 0$ sets the extra confidence placed on positive interactions. One threshold $\theta=3$ defines positives across CPARMS construction, SPPMI construction, WMF training, and evaluation relevance, following the explicit-to-implicit preprocessing of CoFactor~\cite{liang2016cofactor}.

The training objective combines a confidence-weighted reconstruction term over all user--item pairs, a $\gamma$-weighted reconstruction term over signal-supported pairs, and an $\ell_2$ penalty,
\begin{equation}
\begin{aligned}
\mathcal{L} =\;
& \sum_{u\in\mathcal{U}}\sum_{i\in\mathcal{I}}
...
\textbf{CoFactor-WMF} implements the model in Section~\ref{sec2}. It shares the WMF item factors with a reconstruction of the item--item SPPMI matrix in Eq.~\eqref{eq:sppmi}. The model uses unit weights on nonzero SPPMI entries, separate context factors, item and context biases, and ALS updates for user, item, context, and bias parameters \cite{liang2016cofactor}.
...
Signal weight $\gamma$    & \{0.5, 1.0, 2.0, 4.0\}             & CPARMS  & signal-reconstruction weight \\
...
Performance shows no clear pattern across datasets. Music has the smallest CPARMS-WMF margin over Standard-WMF ($+6.4\%$), while Industrial \& Scientific has the largest ($+58.4\%$). The strongest competitor for users with one positive interaction also varies by domain: CoFactor-WMF on Luxury Beauty and Music, while popularity remains difficult to beat for these users on Industrial \& Scientific and Instruments. Pantry is the only dataset where CPARMS-WMF has the best mean for users with one positive interaction.
...
\begin{enumerate}
\item \textbf{Popularity stays competitive at the shortest histories.} CPARMS-WMF improves over Standard-WMF and CoFactor-WMF in most comparisons for users with one positive interaction. However, MostPop has the best mean for users with one positive interaction on Industrial \& Scientific and Instruments and the best mean for users with two positive interactions on Industrial \& Scientific. As a result, the method reduces the performance loss of the WMF baselines at short histories but does not always beat a non-personalized baseline.
...
\item \textbf{Limited base-model integration.} The main contribution of this study is the CPARMS signal, which is designed to augment a base recommender; in this work, we integrate it only with WMF. We have not evaluated whether CPARMS can be combined effectively with other architectures, such as graph-based or neural-based. The reported results therefore establish the effectiveness of CPARMS with WMF but do not yet demonstrate that it generalizes across base models.
...
We examined whether interaction data alone can improve personalized recommendation under sparse implicit feedback. The proposed \textbf{CPARMS-WMF} framework clusters users and items from positive feedback, mines single-antecedent association rules over user-cluster, liked-item-cluster, and item-level tokens, and reconstructs the resulting signal through an additional ALS-optimized term.

Overall, CPARMS-WMF achieves higher ranking accuracy than both WMF baselines under the evaluated protocol and outperforms MostPop at the dataset level.

The advantage weakens for users with one positive interaction, particularly relative to MostPop.

Future work should first establish a fair protocol for users with no positive interactions. A component-wise ablation could then separate the contributions of user-cluster, liked-item-cluster, and item-level antecedents. Future studies should also integrate the CPARMS signal with other base recommenders, including graph-based and neural matrix-factorization models, to determine whether its benefits extend beyond WMF. Evaluation beyond NDCG should also examine whether the gains persist for diversity, novelty, and calibration.
...
\bmhead{Funding}
This work was supported by King Mongkut's Institute of Technology Ladkrabang (KMITL) and KASIKORN Business-Technology Group (KBTG).
...
The Amazon review datasets used in this study are publicly available from the McAuley lab data repository~\cite{ni2019justifying}. Section~\ref{sec4} fully describes the experimental configurations, including the hyperparameter search space and the evaluation protocol, to support reproducibility.
...
P.T., A.L., and A.S. contributed to the study and manuscript preparation. All authors reviewed and approved the final manuscript.
